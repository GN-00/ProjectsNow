...
{
    public partial class Panels_Production_Window : Window
    {
        public User UserData { get; set; }
        public JobOrder JobOrderData { get; set; }
        public ObservableCollection<JPanel> PanelsData { get; set; }

        readonly string filter1 = Statuses.Waiting_Approval.ToString();
        public Panels_Production_Window()
        {
            InitializeComponent();
        }
        ...
        private void Action_Click(object sender, RoutedEventArgs e)
        {
            if (List1.SelectedItem is JPanel panelData)
            {
                ...
                panelData.DateOfProduction = DateTime.Today;
        ...
        private void Undo_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (panelData.Status == Statuses.Production.ToString())
                {
                    if (panelData.ClosedQty == 0)
                    {
                        panelData.Status = Statuses.Waiting_Approval.ToString();
                        panelData.DateOfProduction = null;
                ...
                {
                    if (panelData.Status == Statuses.Closed.ToString())
                        CMessageBox.Show("Panels", "The panel closed!!", CMessageBoxButton.OK, CMessageBoxImage.Warning);
        ...
        private void Data2Filter(object sender, FilterEventArgs e)
        {
            try
            {
                ...
                {
                    if (panel.DateOfProduction == null)
                    {
                        e.Accepted = false;
                        return;
                    }
